...
const { convertToSticker, getMediaBuffer } = require('./stickerHandler')
...
async function handleCommand(sock, msg) {
  ...
  try {
    switch (cmd) {
      ...
      // ── !help ────────────────────────────────────────────────────────────
      case 'help':
        await reply(sock, senderJid, msg,
          `🤖 *Reo'sBot Commands*\n\n` +
          `📋 *Reminder*\n` +
          `!cek — reminder di konteks ini\n` +
          `!cek semua — semua reminder (pribadi)\n` +
          `!cek grup — semua reminder grup (pribadi)\n` +
          `!hari — reminder hari ini\n` +
          `!kirim — kirim reminder sekarang\n` +
          `!done [no] — tandai selesai\n` +
          (requireOwnerApproval()
            ? `!approve [code] — ${approverLabel()} setujui done\n` +
              `!reject [code] — ${approverLabel()} tolak done\n`
            : '') +
          `\n🎭 *Fun*\n` +
          `!damn — kirim sticker pilihan\n` +
          `!setdamn — set sticker (reply sticker/gambar)\n` +
          `!tambah — tambah reminder baru\n` +
          `!edit [no] [field] [nilai] — ubah satu field\n` +
          `!hapus [no] — hapus reminder\n` +
          `!summary — ringkasan semua reminder\n\n` +
          `🎨 *Sticker*\n` +
          `Kirim gambar ke bot → jadi sticker\n` +
          `Di grup: kirim gambar + caption !sticker\n\n` +
          `ℹ️ *Info*\n` +
          `!status — info bot\n` +
          `!help — pesan ini`
        )
      ...
      case 'setdamn': {
        ...
        const outBuffer = isSticker ? mediaBuffer : await convertToSticker(mediaBuffer)
      ...
      case 'reject': {
        ...
        if (cmd === 'reject') {
          ...
          await sock.sendMessage(
            pending.replyJid,
            { text: `🚫 Permintaan !done *[${pending.globalNo}] ${pending.task}* ditolak oleh ${displayNameFromJid(fromJid)}.` }
          )
          break
        }
        ...
        const found = reminders.find((r) => r.key === pending.reminderKey)
        if (!found) {
          pendingDoneApprovals.delete(code)
        ...
        const result = await markAsDone(found, { updatedBy: fromJid, approvedByName: approverName })
        pendingDoneApprovals.delete(code)
        ...
        if (result.success) {
          ...
          await sock.sendMessage(
            pending.replyJid,
            { text: `✅ Disetujui oleh ${approverName}. *[${pending.globalNo}] ${pending.task}* ditandai selesai.` }
          )
        } else {
      ...
      case 'cek': {
        ...
        if (filtered.length === 0) {
          const hint = isFromGroup(msg)
            ? 'Tidak ada reminder untuk grup ini.'
      ...
      case 'done': {
        ...
        const who = displayNameFromJid(fromJid)
        const result = await markAsDone(found, { updatedBy: fromJid, approvedByName: who })
        if (result.success) {
          const modeText = result.mode === 'override' ? ' (override auto-import)' : ''
          await reply(sock, senderJid, msg, `✅ *[${targetNo}] ${found.task}* ditandai selesai oleh ${who}!${modeText}`)
        } else {
      ...
      case 'edit': {
        ...
        if (result.success) {
          await reply(sock, senderJid, msg,
            `✅ *[${editNo}] ${found.task}* berhasil diupdate!\n\n` +
            `📝 ${editField}: *${editValue}*`
          )
